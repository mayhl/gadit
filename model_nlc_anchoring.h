...
#ifndef MODEL_NLC_ANCHORING
#define MODEL_NLC_ANCHORING

// Imports base model_parameters template.
#include "solver_template.h"

namespace model_nlc_anchoring
{
	// If copying to make new model, change to new model id.
	const model::id ID = model::NLC_ANCHORING;

	const std::string anchoringFile = "anchoringPattern.bin";
	...
	template <typename DATATYPE > struct model_parameters<DATATYPE, ID>
	{
		...
		{
			std::string output;
			output = format_parameter_output::make_title("NLC Anchoring Model");
			...
			output += "lambda = " + format_parameter_output::datatype(this->lambda) + "\n";
			output += "nu     = " + format_parameter_output::datatype(this->nu)     + "\n";
			output += "\n";
